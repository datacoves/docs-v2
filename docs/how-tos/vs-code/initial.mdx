...
[Azure DevOps SSH Key Configuration Instructions](https://learn.microsoft.com/en-us/azure/devops/repos/git/use-ssh-keys-to-authenticate#step-2-add-the-public-key-to-azure-devops). Follow only **Step 2: Add the public key to Azure DevOps**, substeps 1–4. Paste the public key you copied from Datacoves into the **Public Key Data** field. You can skip substeps 5–7.
...
:::note
To enable key-pair authentication, your admin must select `Inferred from user info using a custom template` when setting up the [Connection Template](/docs/how-tos/datacoves/how_to_connection_template). The Snowflake username must match the username associated with the email used to authenticate with Datacoves, e.g., `some_user` would be the Snowflake username for `some_user@example.com`.
:::
